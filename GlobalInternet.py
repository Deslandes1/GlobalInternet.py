...
if not st.session_state.connected:
    ...
    with st.expander("⚙️ System Status (Owner Only)", expanded=False):
        ...
        if owner_pass == "OwnerSpace2025":  # <-- Password updated here
            ...
            with col_cred1:
                ...
                owner_email = st.text_input("Email Address", value=owner.get("email", ""))
            
            with col_cred2:
                owner_bank = st.text_input("Bank Name (Optional)", value=owner.get("bank", ""))
                owner_phone = st.text_input("Phone Number", value=owner.get("phone", "50947385663"))
                owner_currency = st.selectbox("Currency", ["HTG - Haitian Gourde", "USD - US Dollar"], index=0 if owner.get("currency")=="HTG - Haitian Gourde" else 1)
            ...
            with col_a1:
                ...
                if enable_auto:
                    threshold = st.number_input("Auto-pay when balance exceeds ($)", min_value=10.0, value=owner.get("auto_threshold", 50.0))
