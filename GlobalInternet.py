...
Version: 74.0.2 (Fixed owner_state table and signup issues)
"""
...
def sign_up_email(email, password, full_name):
    if supabase is None:
        st.session_state.last_error = "Registration unavailable (Supabase not configured)."
        return False
    try:
        ...
        if user.user:
            ...
            if user.user.identities and len(user.user.identities) > 0:
                st.success("Sign-up successful! Please check your email to confirm your account before logging in.")
            else:
                st.success("Sign-up successful! You can now log in.")
            return True
        else:
            st.session_state.last_error = "Sign-up failed: No user returned."
            return False
    except Exception as e:
        error_str = str(e)
        if "User already registered" in error_str:
            ...
            st.session_state.last_error = f"Sign-up failed: {error_str}"
        return False

def log_in_email(email, password, remember=False):
    if supabase is None:
        st.session_state.last_error = "Login unavailable (Supabase not configured)."
        return
    try:
        ...
        if "Invalid login credentials" in error_str:
            ...
            st.session_state.last_error = f"Login failed: {error_str}"
...
# (All rendering functions remain the same as in the previous version)
# For brevity, I'm not repeating them here – they are identical to the ones in the last full version.
# Please keep your existing render_live_page, render_feed, render_friends_page, render_map, render_profile, etc.
...
# --- Login Interface (with improved error messages) ---
def login_interface():
