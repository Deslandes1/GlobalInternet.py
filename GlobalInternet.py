    ...
    with st.form("real_withdrawal_form"):
        ...
        if method == "MonCash":
            phone_number = st.text_input(
                ...
                value=MONCASH_NUM  # Pre-filled with your business number
            )
        else:  # Bank Transfer
        ...
        submitted = st.form_submit_button("💸 Withdraw Funds", use_container_width=True)
        
        if submitted:
            if amount <= 0:
                st.error("Please enter a valid amount.")
            elif amount > st.session_state.real_balance:
                st.error("Insufficient balance.")
            elif method == "MonCash" and not phone_number:
                st.error("Please enter your MonCash phone number.")
            elif method == "Bank Transfer" and not bank_details:
                st.error("Please fill in all bank details.")
            else:
                with st.spinner("Processing withdrawal..."):
                    success, tx_id, msg = initiate_withdrawal(
                        amount, method, 
                        phone_number=phone_number, 
                        bank_details=bank_details
                    )
                    if success:
                        st.session_state.current_transaction = tx_id
                        st.session_state.withdrawal_in_progress = True
                        st.success(f"✅ {msg} (Transaction ID: {tx_id})")
                        # Deduct from local balance after successful initiation
                        st.session_state.real_balance -= amount
                        st.rerun()
                    else:
                        st.error(f"❌ Withdrawal failed: {msg}")
    
    # Show pending transaction status
    if st.session_state.withdrawal_in_progress and st.session_state.current_transaction:
        st.divider()
        st.subheader("📋 Transaction Status")
        tx_id = st.session_state.current_transaction
        success, status, details = verify_transaction(tx_id)
        
        if success:
            if status == "completed":
                st.markdown(f"<div class='transaction-success'>✅ Transaction {tx_id} completed successfully.</div>", unsafe_allow_html=True)
                if st.button("Clear completed transaction"):
                    st.session_state.withdrawal_in_progress = False
                    st.session_state.current_transaction = None
                    st.rerun()
            elif status == "pending":
                st.markdown(f"<div class='transaction-pending'>⏳ Transaction {tx_id} is pending. Check back later.</div>", unsafe_allow_html=True)
                if st.button("Check again"):
                    st.rerun()
            else:
                st.info(f"Transaction status: {status}")
        else:
            st.warning("Could not verify transaction status.")
    
    st.divider()
    
    # --- Transaction History Placeholder ---
    st.subheader("📜 Recent Transactions")
    st.info("Transaction history will appear here once integrated with your backend.")
